<?php

use MetaRush\DataAccess;
use PHPUnit\Framework\TestCase;

class AtlasQueryAdapterTest extends TestCase
{
    private $cfg;
    private $dal;
    private $usersTable;
    private $pdo;

    public function setUp(): void
    {
        $this->usersTable = 'Users';

        $dsn = 'sqlite::memory:';

        $tablesDefinition = [
            'Users' => [
                'id', 'firstName', 'lastName', 'age'
            ]
        ];

        // ------------------------------------------------

        $this->cfg = (new DataAccess\Config)
            ->setDsn($dsn)
            ->setTablesDefinition($tablesDefinition)
            ->setStripMissingColumns(true)
            ->setLogger(new Psr\Log\NullLogger());
            //->setLogger($this->getFileLogger()); // uncomment to actually log files for debugging

        $adapter = new DataAccess\Adapters\AtlasQuery($this->cfg);
        $this->dal = new DataAccess\DataAccess($adapter);

        // ------------------------------------------------

        $this->dal->query('
                CREATE TABLE `' . $this->usersTable . '` (
                `id`        INTEGER PRIMARY KEY AUTOINCREMENT,
                `firstName`	TEXT,
                `lastName`	TEXT,
                `age`       INTEGER
            )');

        // ------------------------------------------------

        $this->seedTestData();
    }

    public function tearDown(): void
    {
        // close the DB connections so unlink will work
        unset($this->dal);
        unset($this->pdo);
    }

    public function seedTestData()
    {
        $data = [
            ['firstName' => 'Foo', 'lastName' => 'Bar', 'age' => 20],
            ['firstName' => 'Jane', 'lastName' => 'Doe', 'age' => 30],
            ['firstName' => 'John', 'lastName' => 'Doe', 'age' => 40],
            ['firstName' => 'Bar', 'lastName' => 'Refaeli', 'age' => 25]
        ];

        foreach ($data as $v)
            $this->dal->create($this->usersTable, $v);
    }

    private function getFileLogger(): Psr\Log\LoggerInterface
    {
        return (new \Monolog\Logger('MRDA'))
                ->pushHandler(
                    new Monolog\Handler\RotatingFileHandler(__DIR__ . '/../log.txt', 2)
                );
    }

    public function testCreate()
    {
        $insertId = $this->dal->create($this->usersTable, ['firstName' => 'Quz', 'lastName' => 'Test']);

        $this->assertIsInt($insertId);

        $this->assertEquals(5, $insertId);

        $this->assertEquals(5, $this->dal->getLastInsertId());
    }

    public function testFindOne()
    {
        $row = $this->dal->findOne($this->usersTable, ['firstName' => 'Foo']);

        $this->assertIsArray($row);

        $this->assertEquals('Foo', $row['firstName']);
    }

    public function testFindColumn()
    {
        $column = $this->dal->findColumn($this->usersTable, ['firstName' => 'Bar'], 'lastName');
        $this->assertEquals('Refaeli', $column);

        $column = $this->dal->findColumn($this->usersTable, ['firstName' => 'Jane'], 'age');

        $this->assertEquals(30, $column);
    }

    public function testFindAllWithWhere()
    {
        $rows = $this->dal->findAll($this->usersTable, ['firstName' => 'John']);

        $this->assertIsArray($rows);

        $this->assertEquals('John', $rows[0]['firstName']);
    }

    public function testFindAllUsingOtherWhereOperators()
    {
        $rows = $this->dal->findAll($this->usersTable, ['age > 20']);
        $this->assertCount(3, $rows);

        $rows = $this->dal->findAll($this->usersTable, ['age >= 30']);
        $this->assertCount(2, $rows);

        $rows = $this->dal->findAll($this->usersTable, ['age BETWEEN 21 AND 39']);
        $this->assertCount(2, $rows);

        $rows = $this->dal->findAll($this->usersTable, ["firstName LIKE 'J%'"]);
        $this->assertCount(2, $rows);
    }

    public function testFindAllWithoutWhere()
    {
        $rows = $this->dal->findAll($this->usersTable);

        $this->assertIsArray($rows);

        $this->assertEquals('Foo', $rows[0]['firstName']);
        $this->assertEquals('Jane', $rows[1]['firstName']);
        $this->assertEquals('Doe', $rows[2]['lastName']);
    }

    public function testUpdateWithWhere()
    {
        $where = ['firstName' => 'John'];
        $data = ['firstName' => 'Jane'];
        $this->dal->update($this->usersTable, $data, $where);

        $row = $this->dal->findOne($this->usersTable, ['firstName' => 'Jane']);

        $this->assertEquals('Jane', $row['firstName']);
    }

    public function testUpdateWithoutWhere()
    {
        $data = ['firstName' => 'Jane'];
        $this->dal->update($this->usersTable, $data);

        $row = $this->dal->findOne($this->usersTable, ['firstName' => 'Jane']);

        $this->assertEquals('Jane', $row['firstName']);
    }

    public function testDeleteWithWhere()
    {
        $where = ['firstName' => 'Jane'];
        $this->dal->delete($this->usersTable, $where);

        $row = $this->dal->findOne($this->usersTable, ['firstName' => 'Jane']);

        $this->assertEquals(null, $row);
    }

    public function testDeleteWithoutWhere()
    {
        $this->dal->delete($this->usersTable);

        $row = $this->dal->findAll($this->usersTable);

        $this->assertEquals([], $row);
    }

    public function testFindAllOrderBy()
    {
        $rows = $this->dal->findAll($this->usersTable, [], 'age DESC');
        $this->assertEquals(40, $rows[0]['age']);

        $rows = $this->dal->findAll($this->usersTable, [], 'firstName');
        $this->assertEquals('Bar', $rows[0]['firstName']);
    }

    public function testFindAllLimitAndOffset()
    {
        $rows = $this->dal->findAll($this->usersTable, [], null, 2);
        $this->assertCount(2, $rows);

        $rows = $this->dal->findAll($this->usersTable, [], null, 2, 2);

        $this->assertCount(2, $rows);
        $this->assertEquals('John', $rows[0]['firstName']);
    }

    public function testTransactions()
    {
        // test rollback
        $this->dal->beginTransaction();
        $data = ['firstName' => 'Alice'];
        $this->dal->create($this->usersTable, $data);
        $data = ['firstName' => 'Bob'];
        $this->dal->create($this->usersTable, $data);
        $this->dal->rollBack();

        $where = ['firstName' => 'Alice'];
        $rows = $this->dal->findOne($this->usersTable, $where);
        $this->assertNull($rows);

        $where = ['firstName' => 'Bob'];
        $rows = $this->dal->findOne($this->usersTable, $where);
        $this->assertNull($rows);

        // test commit
        $this->dal->beginTransaction();
        $data = ['firstName' => 'Alice'];
        $this->dal->create($this->usersTable, $data);
        $data = ['firstName' => 'Bob'];
        $this->dal->create($this->usersTable, $data);
        $this->dal->commit();

        $where = ['firstName' => 'Alice'];
        $rows = $this->dal->findOne($this->usersTable, $where);
        $this->assertEquals('Alice', $rows['firstName']);

        $where = ['firstName' => 'Bob'];
        $rows = $this->dal->findOne($this->usersTable, $where);
        $this->assertEquals('Bob', $rows['firstName']);
    }

    public function testStripMissingColumnsCreateAndUpdate()
    {
        $data = [
            'id'                => 9,
            'firstName'         => 'Foo',
            'lastName'          => 'Bar',
            'age'               => 10,
            'nonExistentColumn' => 'qux'
        ];

        $this->dal->create($this->usersTable, $data);

        $where = ['id' => 9];
        $row = $this->dal->findOne($this->usersTable, $where);
        $this->assertEquals(10, $row['age']);

        // ------------------------------------------------

        $data = [
            'age'                      => 11,
            'anotherNonExistentColumn' => 'qux'
        ];

        $where = ['id' => 9];

        $this->dal->update($this->usersTable, $data, $where);

        $row = $this->dal->findOne($this->usersTable, $where);
        $this->assertEquals(11, $row['age']);
    }

    public function testStripMissingColumnsMissingTableInDefinition()
    {
        $this->expectExceptionMessageMatches('/not defined in your tables definition/');

        $this->dal->create('nonExistentTable', []);
    }

    public function testStripMissingColumnsAsServiceMethod()
    {
        $this->dal->setStripMissingColumns(false);

        $val = $this->cfg->getStripMissingColumns();

        $this->assertFalse($val);
    }

    public function testFindAllWithGroupBy()
    {
        $this->dal->groupBy('lastName');
        $rows = $this->dal->findAll($this->usersTable);

        $this->assertCount(3, $rows);
    }

    public function testQueryWithCustomSql()
    {
        $preparedStatement = 'SELECT * FROM ' . $this->usersTable . ' WHERE age < ?';
        $bindParams = [30];
        $rows = $this->dal->query($preparedStatement, $bindParams);

        $this->assertCount(2, $rows);

        $this->assertEquals('Foo', $rows[0]['firstName']);
        $this->assertEquals('Bar', $rows[1]['firstName']);
    }

    public function testExecWithSingleInsert()
    {
        $preparedStatement = "INSERT INTO $this->usersTable (firstName, lastName, age) VALUES (?, ?, ?)";
        $bindParams = ['Mark', 'Calaway', '31'];
        $numRows = $this->dal->exec($preparedStatement, $bindParams);

        $this->assertEquals(1, $numRows);
    }

    public function testExecWithMultipleInsert()
    {
        $preparedStatement = "INSERT INTO $this->usersTable (firstName, lastName, age) VALUES (?, ?, ?), (?, ?, ?)";
        $bindParams = ['Mark', 'Calaway', null, 'Dwayne', 'Johnson', '32'];
        $numRows = $this->dal->exec($preparedStatement, $bindParams);

        $this->assertEquals(2, $numRows);

        // ------------------------------------------------

        /* also test getLastInsertId() */
        $this->assertEquals(6, $this->dal->getLastInsertId());
    }

    public function testExecWithUpdate()
    {
        $preparedStatement = "UPDATE $this->usersTable SET age = ? WHERE lastName = 'Doe'";
        $bindParams = ['18'];
        $numRows = $this->dal->exec($preparedStatement, $bindParams);

        $this->assertEquals(2, $numRows);
    }

    public function testExecWithDelete()
    {
        $preparedStatement = "DELETE FROM $this->usersTable WHERE lastName = ?";
        $bindParams = ['Doe'];
        $numRows = $this->dal->exec($preparedStatement, $bindParams);

        $this->assertEquals(2, $numRows);
    }
}